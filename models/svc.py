...
x_train,y_train = regex_x, train['label']

test_titles = test['title1_en']+test['title2_en']
x_test = [re.sub(r'(.)\1{2,}', r'\1', title) for title in test_titles]
# %%
...
submit_df = pd.DataFrame({'id':test_df['id'], 'label':y_pred_SVC})
submit_df.to_csv('submission_tfidf_SVC.csv',index=False)
